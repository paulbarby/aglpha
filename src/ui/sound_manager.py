...
class SoundManager:
    ...
    def play_music(self, name, fadeout=0, loops=-1):
        ...
        if name in self.music_tracks and self.music_tracks[name] is not None:
            ...
            if fadeout > 0:
                pygame.mixer.music.fadein(fadeout)  # Use pygame's fadeout if specified
            else:
    ...
    def stop_music(self, force=False, fadeout=0):
        """Stop currently playing music."""
        if not self.music_enabled and not force:
            return
        
        if fadeout > 0:
            pygame.mixer.music.fadeout(fadeout)  # Use pygame's fadeout if specified
        else:
